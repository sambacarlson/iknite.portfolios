...
const ProfileCard = (props:{member:any}) => {
  ...
  return (
    <Link to="/memberProfile" state={{from:fromData}} onMouseOver={()=>handleHover(true)} onMouseOut={()=>handleHover(false)} >
      ...
      <div className='flex flex-row items-center justify-start px-2 space-x-3 md:hidden' >
        ...
        <div className="flex flex-col items-start space-y-2 w-1/2">
          <h3 className={`border-b-[1px] border-${uiColor? 'dark' : 'reddish'} text-dark pl-1 pr-12 font-bold mb-1 whitespace-nowrap`}>{props.member.memberName.toUpperCase()}</h3>
      ...
      <div className='hidden md:flex flex-col items-center justify-start px-5 space-y-3' >
        ...
        <div className="flex flex-col items-start space-y-1 w-full">
          <p className='flex flex-row gap-2 item-center w-full'>
            <h3 className={`border-b-[1px] border-${!uiColor? 'reddish' : 'dark'} text-dark w-full py-1 pl-2 font-bold mb-1 whitespace-nowrap`}>{props.member.memberName.toUpperCase()}</h3>
          </p>
