    ...
    <Link to="/memberProfile" onMouseOver={()=>handleHover(true)} onMouseOut={()=>handleHover(false)} >
      ...
      <div className='flex flex-row items-center justify-start px-5 space-x-3 md:hidden' >
        ...
        <div className="flex flex-col items-start space-y-2 w-1/2">
          <h3 className={`bg-${uiColor? 'dark' : 'reddish'} text-white pl-1 pr-12 font-bold shadow-tobottom mb-1`}>{`Carlson`.toUpperCase()}</h3>
          <h4 className='text-dark'>Developer</h4>
      ...
      <div className='hidden md:flex flex-col items-center justify-start px-5 space-y-3' >
        ...
        <div className="flex flex-col items-start space-y-1 w-full">
          <p className='flex flex-row gap-2 item-center w-full'>
            <h3 className={`bg-${uiColor? 'reddish' : 'dark'} text-white w-full py-1 pl-2 font-bold shadow-tobottom mb-1`}>{`Carlson`.toUpperCase()}</h3>
          </p>
