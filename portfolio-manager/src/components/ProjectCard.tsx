...
export const ProjectCard = () => {
  ...
  return (
    <Link to="/projectProfile" onMouseOver={()=>handleHover(true)} onMouseOut={()=>handleHover(false)} className={`bg-white ${uiColor? 'text-dark' : 'text-reddish'} p-2 m-1 grid grid-cols-3 gap-1 w-full h-full`}>
      ...
      <div className='col-span-2 p-1 flex flex-col space-y-3 items-center justify-center'>
        ...
        <div className='flex flex-row -space-x-2 items-center, justify-start pt-3'>
          <div className='w-10 h-10 bg-light flex items-center justify-center rounded-full text-dark shadow-toleft ring-1 ring-light'>
          ...
          <div className='w-10 h-10 bg-white flex items-center justify-center rounded-full text-dark shadow-toleft ring-1 ring-light'>
            <p>4+</p>
          </div>
        </div>
      </div>
